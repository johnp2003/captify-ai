...
export async function getUserPoints(userId: string) {
  try {
    console.log('Getting user points for user', userId);
    const users = await db
      .select({ points: Users.points, id: Users.id, email: Users.email })
      .from(Users)
      .where(eq(Users.stripeCustomerId, userId))
      .execute();
    console.log('User points', users);
    if (users.length === 0) {
...
export async function createOrUpdateSubscription(
  ...
  try {
    const [user] = await db
      .select({ id: Users.id })
      .from(Users)
      .where(eq(Users.stripeCustomerId, userId.toString()))
      .limit(1);

    if (!user) {
      console.error('User not found with stripeCustomerId:', userId);
      return null;
    }
